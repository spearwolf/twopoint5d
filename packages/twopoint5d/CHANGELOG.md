...
- add the `'texture'` value to the `source` field of `TextureResource`'s `error` event: a failure behind an image that already loaded — texture creation itself, or any value derived from it, such as an atlas or a tile set — reports `{source: 'texture', id, error}`, naming the resource instead of a url that never failed. A subscriber that throws while the resource publishes what it loaded is reported the same way, and the value stays published
...
- an image or a tile set `TextureResource` whose `imageUrl` is cleared — directly, or by a `TextureStore#parse()` whose tile set item carries none — takes back its `imageCoords`, its `texture` and what was built from them: the `tileSet`, its `atlas` and the `frameBasedAnimations`. It releases the texture it built. The getters answer `undefined`, the retained events are cleared, and a later `TextureStore#on()` or `TextureStore#get()` waits for the next image. A `textureFactory` that is cleared takes nothing back. An atlas `TextureResource` whose `atlasJson` names no image, with no `overrideImageUrl` to fall back on, has no `imageUrl` either and takes back its `imageCoords`, its `texture`, its `atlas` and the `frameBasedAnimations` the same way
...
- `VOBufferGeometry#dispose()` and `InstancedVOBufferGeometry#dispose()` release exactly the pools the geometry created itself, and leave every pool that was handed to the constructor untouched. Both also take the attributes built on the pool buffers off the geometry and drop the index, so nothing keeps the typed arrays alive through the geometry; attributes copied from a `BufferGeometry` passed to `InstancedVOBufferGeometry` stay where they are, because no pool feeds them and nothing has to take them off — `copy()` cloned them, so they and their typed arrays belong to the geometry and fall with it, while the geometry they were cloned from stays untouched. A further `update()` on a disposed geometry finds no attributes left to write to — it still sets the draw range and, on `InstancedVOBufferGeometry`, `instanceCount`
...
- `Display#renderer`, `#frameLoop` and `#frameNo` are read-only accessors on the prototype: a write is a type error, and at runtime it throws a `TypeError` in strict-mode code — every ES module and class body — and is ignored elsewhere. The display owns the renderer and the frame loop and releases both in `dispose()`, and it counts `frameNo` itself — see the migration guide
- `FrameLoop#frameNo`, `#now`, `#deltaTime` and `#measuredFps` are read-only accessors on the prototype: a write is a type error, and at runtime it throws a `TypeError` in strict-mode code — every ES module and class body — and is ignored elsewhere. The loop counts and measures them itself; `now` and `deltaTime` are in milliseconds, the props of `FrameLoop.OnFrame` carry both in seconds — see the migration guide
...
- deprecate the instance method `TextureStore#load()` and the static `TextureStore.load()` in favour of `loadAsync()`. The instance `load()` resolves with the store however the attempt ends. The old names stay until a breaking release removes them
- deprecate `TextureStore#get()` in favour of `getAsync()`; its messages name `get()`. The old name stays until a breaking release removes it
...
- fix a listener of `OnDisplayStart` that throws: every listener of `OnDisplayStart` still hears the event, then the display goes into the pause as a `stop()` would put it there — `OnDisplayPause` follows, the display is off its frame loop, `pause` answers `true`, and a tab that becomes visible again does not start it — and `Display#start()` rejects with the error, or with an `AggregateError` when more than one listener throws. Before, the display went on running while `start()` rejected, and the listeners behind the one that threw never heard the start. The next `start()` emits `OnDisplayRestart` and `OnDisplayStart`
- fix a listener of `OnDisplayPause` that throws: every listener of `OnDisplayPause` still hears the event, and `pause = true` or `Display#stop()` throws its error afterwards — an `AggregateError` when more than one listener throws. When that pause follows a listener of `OnDisplayStart` that throws, `Display#start()` rejects with an `AggregateError` whose `errors` carry the error of the start and the error of the pause
- fix a tick of the animation loop of three without a timestamp — the one three runs right away as it starts its loop, at the end of an XR session among others: it reaches no `FrameLoop` and goes into no fps measurement
- fix `Display#dispose()` with a listener that throws: a listener of `OnDisplayPause` — a running display pauses first — or of `OnDisplayDispose` that throws does not stop the teardown. Every listener of `OnDisplayDispose` hears the event, the display comes off its frame loop and drops every listener, a canvas handed in goes back as the display found it, the renderer is released and a container the display built leaves the DOM; only then does `dispose()` throw the error — an `AggregateError` when more than one listener of the event throws, and an `AggregateError` of the pause error and the dispose error when listeners of both events throw. A `Display` constructor that fails and meets such a listener as it takes itself down again throws an `AggregateError` of its own error and that of the listener
- fix `Display#dispose()` of a WebGPU renderer while the page is hidden: the release no longer gives up waiting for two animation frames after two seconds in a hidden tab, where the page draws none, and destroys the device before the page has presented the canvas. It waits until the page is visible again and has drawn two frames, also when the page is hidden only while the release waits; the two seconds count only while the page is visible. A display built meanwhile on a canvas handed in waits as well
...
- fix `Display#dispose()` for a renderer that is still initializing or still has work on the GPU: the renderer is released once its init is through and the GPU has run the work submitted to it, or once the device reports itself lost, or after two seconds with a warning on the console, so a `dispose()` during the init takes the device, the context and the animation loop of three along, and an init that fails ends without an unhandled rejection. Under WebGPU the release then waits until the page has drawn two more animation frames, or for two more seconds while it draws none, so under Firefox with WebGPU the page keeps its `requestAnimationFrame` when the canvas stays in the document — a canvas handed to the constructor, or that of an adopted `WebGPURenderer`. `dispose()` itself stays synchronous — `renderer` is `undefined` and a container the display built is out of the DOM when it returns, and the renderer is released after that
...
- fix the `update` event of `PanControl2D`: it goes out whenever `update()` moved the `panView`, including a control with both input sources switched off whose `speed…` fields were set by hand, and on the first `update()` after a new state was assigned to `panView`, even when that same state is assigned once more before it
...
- fix `Stage2D#width` and `#height` on a change of `projection`: the size goes to 0 together with the camera of the previous projection, `projection = undefined` included, and stays 0 until the projection in place gives a view with an area. `OnStageResize` announces the first view of the new projection, also when it has the size of the previous one, and sends nothing for the drop to 0
...
- fix `TileSet`: it throws a `RangeError` naming the option and the value for a `tileWidth` or `tileHeight` that is no finite number above 0, for a negative or non-finite `margin`, `padding` or `spacing`, for a `tileCount` that is no whole number of 1 or more, and for a non-finite width or height of the `baseCoords` — a typo in a catalog ends as a `RangeError`, and a `TextureResource` reports it as an `error` with `source: 'texture'`
...
- fix a tile set `TextureResource` whose `tileSetOptions` `TileSet` refuses or that are cleared: it offers no `tileSet`, no `atlas` and no `frameBasedAnimations` built from other options — the getters answer `undefined`, the retained events are cleared instead of carrying `undefined`, and a later `TextureStore#on()` or `TextureStore#get()` waits for the next tile set. A refusal is reported as an `error` with `source: 'texture'` whether the image arrives or the options change, so neither the `tileSetOptions` setter nor `TextureStore#parse()` throws
- fix `TileSet` with a fractional `padding`: each tile is laid out by twice the whole padding, so a `padding` of `1.5` steps tiles of `10` px by `13` px and fits as many as the image holds
- fix `TextureResource#frameBasedAnimations`: it is taken back once `frameBasedAnimationsData` is cleared, also by a `TextureStore#parse()` whose item carries no animation data — the getter answers `undefined` and the retained event is cleared
- fix an atlas `TextureResource` whose `atlasJson` is cleared or cannot be read: it offers no `atlas` and no `frameBasedAnimations` built from it, and the texture stays. A json that `TexturePackerJson` cannot read is reported as an `error` with `source: 'texture'` once the image it names is there; the `atlasJson` setter does not throw
- fix `TextureResource`: every subscriber of `imageCoords`, `atlas`, `tileSet`, `texture` and `frameBasedAnimations` is called and the retained value is written, also behind a subscriber that throws, and the throw reaches the code that wrote the value once every subscriber has heard it; a tile set and its atlas are published together, and a throw while the resource publishes what it loaded arrives as an `error` with `source: 'texture'`
...
- fix `Canvas2DStage#setContainerSize()`: the size goes through the public `stageRenderer`, whose `width` and `height` then hold the container size instead of staying `0`. A `pipeline` set on that renderer gets an internal render target of the container size rather than the 1×1 minimum. From there the renderer decides whether anything moves, and it weighs its own size against the size of every stage item: a call with the size it already answers with still reaches the stages that have not taken it yet, and leaves the render target and the stages that carry it alone.
...
An upload out of a pool carries the objects that were written, and `getVO()` marks nothing at all: reading a slot changes no data, while the marking it did made a single `getVO()` upload every buffer of the pool over its whole area in use.
...
An image or a tile set resource releases the texture it built once its `imageUrl` is cleared, an
atlas resource once its `atlasJson` names no image and no `overrideImageUrl` is set, and the
`texture` getter answers `undefined`. A caller that needs the texture past that point does not
...
```ts
const image = await new ImageLoader().loadAsync(resource.imageUrl!);
...
`Display#canvas` and `#getEventProps()` throw once `dispose()` has run, and `#nextFrame()` is
...
The three are accessors without a setter: a write is a type error, and at runtime it throws a
`TypeError` in strict-mode code — every ES module and class body — and is ignored elsewhere. The
display owns its renderer and its frame loop and releases both in `dispose()`, and it counts
`frameNo` itself. A renderer of your own goes to the constructor, a subscriber of your own goes onto
the frame loop the display runs on, and `frameNo` is only read.
...
The four are accessors without a setter: a write is a type error, and at runtime it throws a
`TypeError` in strict-mode code — every ES module and class body — and is ignored elsewhere. The
loop counts its frames and measures their time and rate itself. To count frames from a point of
your own, remember the value at that point and subtract it later.
