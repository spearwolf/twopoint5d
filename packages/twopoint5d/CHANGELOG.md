...
- fix `TextureResource`: every subscriber of `imageCoords`, `atlas`, `tileSet`, `texture` and `frameBasedAnimations` is called and the retained value is written, also behind a subscriber that throws, and the throw goes on afterwards; a tile set and its atlas are published together, and a throw while the resource publishes what it loaded arrives as an `error` with `source: 'texture'`
