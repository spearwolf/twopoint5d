...
- fix `StageRenderer#resize()` for a stage that refuses the size: `width` and `height` of the renderer stay the way the call found them, and so does the size of the stage item of every stage that refused — a stage that took the size keeps it, item and all. Each of the other stages is asked for the size all the same. The very same call therefore goes through again as soon as the refusing stage takes the size, and reaches exactly the stages that still owe it. Several stages refusing at once come out as one `AggregateError` naming how many of them it was; a single one comes out unchanged, so a `TypeError` of a projection reaches the caller as a `TypeError`
